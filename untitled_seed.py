...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 27 17:51:55 2022
...
num_classes = 3             #3类
batch_size = 128
img_rows, img_cols, num_chan =19, 19, 4
# img_rows, img_cols, num_chan = 17, 19, 5
people_num = [1,6,7,15,12,10,2,5,8,13,3,14,11,9,4]


falx = np.load("G:\\4情绪识别任务2\\4D-CRNN-master(4区)调整\\SEED\\data\\DE0.5s\\t6x_1919_ji.npy")
y = np.load("G:\\4情绪识别任务2\\4D-CRNN-master(4区)调整\\SEED\\data\\DE0.5s\\t6y_1919_ji.npy")
      
# one_y = np.array([y[:1697]] * 3).reshape((-1,))
# one_y = to_categorical(one_y, num_classes)
one_y_1 = np.array([y[:1126]] * 3).reshape((-1,))        #shape 3378列,（1126*3,）1126（=6788/6）为1个周期（15个情绪，15种P、O、N）
one_y_1 = to_categorical(one_y_1, num_classes)           #变成二进制形式（3378*3）
acc_list = []
std_list = []
all_acc = []
all1_acc = []
zero1=np.zeros([3378,6,19,19,6])

for nb in range(15):
    
    K.clear_session()
    start = time.time()
    one_falx_1 = falx[nb * 3:nb * 3 + 3]              #取1人的3次（3*1126*6*8*9*5）
    one_falx_1 = one_falx_1.reshape((-1, 6, img_rows, img_cols, 5))   #合并成（3378*6*8*9*5）
    zero1[:,:,:,:,0:5]=one_falx_1


    # ###============= random select ============####
    one_y = one_y_1                                #二进制形式标签（3378*3）
    one_falx = zero1[:,:,:,:,1:5]             #取后4个频段，由one_falx_1成为3378*6*8*9*4

    #print(one_y.shape)
    #print(one_falx.shape)
    # x_train, x_test, y_train, y_test = train_test_split(one_falx, one_y, test_size=0.25)
    #seed = 7
    #np.random.seed(seed)    #调用后生成7组随机数，再调用一次这个程序，之后还是生成7组和之前相同的随机数    
    kfold = StratifiedKFold(n_splits=5, shuffle=True)    #5折交叉验证，依照标签的比例来抽取数据,为了保证程序每次运行都分割一样的训练集和测试集
    cvscores = []

    score_hun=None
    # create model1
    for train, test in kfold.split(one_falx, one_y.argmax(1)):    #train大小为2703,test大小为675，one_falx_1为3378*6*8*9*4，one_y.argmax(1)为3378，其中-1变为2
       ...
       # 建立网络共享层
        ...
        # dense layer
        out_layer1_1 = Dense(3, activation = 'softmax', name = 'out')(lstm_layer1_1)
        ...
        # dense layer
        out_layer1_2 = Dense(3, activation = 'softmax', name = 'out')(lstm_layer1_2)
        ...
        out_layer1_3 = Dense(3, activation = 'softmax', name = 'out')(lstm_layer1_3)
        ...
        model3.compile(loss=tensorflow.keras.losses.categorical_crossentropy,
                      optimizer=tensorflow.keras.optimizers.Adam(),
                      metrics=['accuracy'])
        MODEL_DIR = './'+'t'+str(t)+'nb'+str(nb)+state
